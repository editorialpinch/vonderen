	...
	function widget( $args, $instance ) {
		...
		if( is_single() || is_page() ) {
            ...
            if( $related_posts ) {
				...
				echo "<h3 class=\"widget-title orange\">".$instance['title']."</h3>\n";
				echo "<ul>\n";
                foreach( $related_posts as $related_post_id => $related_post_title  ) {
                	echo '<li class="postWidget">';
                	echo '<div class="thumbnailBox">';
                	echo "<a href=\"".get_permalink( $related_post_id )."\">";
                	echo get_the_post_thumbnail($related_post_id,'thumbnail');
                	echo '<span class="postTitle">'.$related_post_title.'</span>';
